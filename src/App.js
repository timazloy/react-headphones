...
function App() {
  ...
  React.useEffect(() => {
    async function fetchData() {
      ...
      const itemsResponse = await axios.get(
        "https://62f2672bb1098f15081212c2.mockapi.io/headphones?" +
          sortPrice +
          "&search=" +
          sortName
      );
      ...
      const cartResponse = await axios.get(
        "https://62f2672bb1098f15081212c2.mockapi.io/cart"
      );
      const orderPageLength = await axios.get(
        `https://639f35a97aaf11ceb8954a67.mockapi.io/Learn`
      );

      const ordersLength = Math.ceil(
        orderPageLength.data.length <= 5 ? 1 : orderPageLength.data.length / 5
      );
  ...
  const addToFavorite = async (obj) => {
    try {
      ...
      if (
        ...
        axios.delete(
          `https://62f2672bb1098f15081212c2.mockapi.io/favorites/${findItem.id}`
        );
        setFavorites((prev) =>
  ...
  const onPlus = async (obj) => {
    ...
    if (findItem) {
      ...
      await axios.delete(
        `https://62f2672bb1098f15081212c2.mockapi.io/cart/${findItem.id}`
      );
    } else {
  ...
  const formOrder = async () => {
    ...
    cartItems.forEach((item) => {
      axios.delete(
        `https://62f2672bb1098f15081212c2.mockapi.io/cart/${item.id}`
      );
    });
  };
