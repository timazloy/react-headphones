...
function OrderPage({ orders, currentDate, getOrders, ordersPage, isLoadingOrders }) {
   return (
      <div className="order-page">
         ...
         {isLoadingOrders
            ? [...Array(5)].map((item, index) => (
                 <Order key={index} item={item} currentDate={currentDate} isLoading={isLoadingOrders} />
              ))
            : orders.map((item) => <Order item={item} currentDate={currentDate} isLoading={isLoadingOrders} />)}

         {ordersPage > 1 && (
            <button onClick={getOrders} className="order-page__show-more" type="button">
               Показать еще
            </button>
         )}
      </div>
